...
public class FileClassDemo {
	public static void main(String[] args) {
		...
		File fileObject = new File("src/Chap10Lab_RE/" + fileName);

		while (fileObject.exists()) {
			System.out.println("There already is a file named " + fileName);
			System.out.println("Enter a different file name:");
			fileName = keyboard.nextLine();
			fileObject = new File("src/Chap10Lab_RE/" + fileName);
		}
		PrintWriter outputStream = null;
		try {
			outputStream = new PrintWriter(new FileOutputStream("src/Chap10Lab_RE/" + fileName));
		}
		catch (FileNotFoundException e) {
			System.out.println("Error opening the file " + fileName);
			System.exit(0);
		}
		System.out.println("Wrting \"" + line + "\"");
